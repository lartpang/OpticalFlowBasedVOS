...
def _read_and_eval_file(mask_video_path: str, pred_video_path: str, frame_name: str):
    ...
    F_score = f_boundary.db_eval_boundary(annotation=binary_frame_mask, segmentation=binary_frame_pred)
...
def convert_all_data_dict_to_table(data_dict: dict, video_name_list: list):
    table = ptable(["Video", "J&F(M)", "J(M)", "J(O)", "J(D)", "F(M)", "F(O)", "F(D)"])
    for video_name in video_name_list:
        table.add_row(
            [video_name, f"{data_dict[video_name]['mean'].mean(): .3f}", ]
            + [
...
def convert_average_data_dict_to_table(data_dict: dict):
    table = ptable(["Video", "J&F(M)", "J(M)", "J(O)", "J(D)", "F(M)", "F(O)", "F(D)"])
    table.add_row(
        ["Average", f"{data_dict['Average']['mean'].mean(): .3f}", ]
        + [
            f"{data_dict['Average'][x][y]: .3f}"
            for y in range(2)
            for x in ["mean", "recall", "decay"]
        ]
    )
...
def eval_method_from_data(
    ...
    # show the results
    if only_average:
        table_str = convert_average_data_dict_to_table(data_dict=average_score_dict)
    else:
        table_str = convert_all_data_dict_to_table(
            data_dict=average_score_dict, video_name_list=eval_video_name_list + ["Average"]
        )
    return table_str
...
# eval_video_name_list = prepare_data_info(data_yaml_path=data_yaml_path)
# eval_method_from_data(eval_video_name_list=eval(, mask_data_root=mask_data_root,)
# show_results_from_data_file()
